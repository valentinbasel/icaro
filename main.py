...
def comprobacion_errores(ventana):
    ...
    if os.path.isdir(dir_conf + "/np05/") == 0:
        # respuesta=men.mensajes(1,"no existe el firmware para icaro-bloques,
        # ¿descargarlo?")
        # print respuesta
        # if respuesta==True:
        try:
            # archivo=urllib.urlretrieve(
            #"http://valentinbasel.fedorapeople.org/firmware/np05.tar.gz",
            # dir_conf+"np05.tar.gz",None)
