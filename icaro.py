...
class Ventana:
    ...
    def __init__(self):
        ...
        menu_general = [
            (_("New"), _("Open"), _("Save"), _("Save as"), _(
             "Save as function"), _("Examples"), _("Exit")),
...
#        entry = gtk.Entry()
#        entry.connect("activate", self.responseToDialog, self.dialog,
#                      gtk.RESPONSE_OK)
#        hbox = gtk.HBox()
...
# ================================================
    ...
    def upload(self, b):
        resultado = 1
        self.mensajes(3,
                      "aprete el boton RESET de la placa pinguino"
                      )
        ...
        if i == 1:
            self.mensajes(
                0, "no se a detectado ningun dispositivo conectada")
            return 1
        if i == 2:
            self.mensajes(
                0, "Se detecto el dispositivo, hay que cargar el firmware")
            return 2
        if i == 2:
    ...
    def guardar(self, dato):
        ...
        if response == gtk.RESPONSE_OK:
            ...
            if os.path.isfile(cadena):
                ...
                resp = False
            if resp is True or os.path.isfile(cadena) is False:
                guardar.guardar(
                    self.fondo.objetos,
                    cadena,
                    self.fondo
...
#        archivo=os.listdir(ruta)
    ...
    def carga_paleta(self):
        R = G = B = ""
        archivo = open(os.path.abspath(os.path.dirname(
            __file__)) + "/colores.dat", "r")
        tupla = []
